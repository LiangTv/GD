...
# --- Git 操作函數 (V9.1.5 - 強化 Git Add 與 Commit 邏輯) ---
def commit_and_push_changes():
    global git_timer
    logging.info("開始執行 Git 操作...")
    try:
        # --- 步驟 1: 使用 'git add .' 來暫存所有變更 ---
        # 這會自動處理所有已修改和未追蹤的檔案 (除了 .gitignore 中指定的)
        # 這比手動組裝列表更可靠。
        ...
        # `git diff --staged --quiet` 如果暫存區有變更，返回 1；如果沒有，返回 0。
        result_status_staged = subprocess.run(['git', 'diff', '--staged', '--quiet'], cwd=REPO_PATH, check=False) # check=False 避免無變更時拋錯

        if result_status_staged.returncode == 0:
             # 如果返回碼是 0，表示暫存區是空的，沒有任何變更需要 commit。
